...
class lime_output
{
  ...
  public function echoln($message, $colorizer_parameter = null, $colorize = true)
  {
    if ($colorize)
    {
      $message = preg_replace_callback('/(?:^|\.)((?:not ok|dubious|errors) *\d*)\b/', function($m) { return $this->colorizer->colorize($m[1], 'ERROR'); }, $message);
      $message = preg_replace_callback('/(?:^|\.)(ok *\d*)\b/', function($m) { return $this->colorizer->colorize($m[1], 'INFO'); }, $message);
      $message = preg_replace_callback('/"(.+?)"/', function($m) { return $this->colorizer->colorize($[1], 'PARAMETER'); }, $message);
