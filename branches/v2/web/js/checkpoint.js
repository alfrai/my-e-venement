function event_checkpoints_list_load(url)
{
  ...
  $('.sf_admin_form .checkpoint_list').load(url+' .sf_admin_list',function(){
    ...
    $('.sf_admin_form .checkpoint_list tfoot a').click(function(){
      event_checkpoints_list_load($(this).attr('href'));
      return false;
    });
    $('.sf_admin_form .checkpoint_list tbody a').click(function(){
      $('.sf_admin_form .checkpoint_new').load($(this).attr('href')+'/edit .sf_admin_form form',event_checkpoints_new_load);
      return false;
    });
  });
}
function event_checkpoints_new_load(data)
{
  ...
  $('.sf_admin_form select[name="checkpoint[event_id]"] option[value='+$('input[type=hidden][name="event[id]"]').val()+']').attr('selected','selected');
  ...
  if ( $('.sf_admin_form select[name="checkpoint[event_id]"]').val() )
  {
    ...
    $('.sf_admin_form select[name="checkpoint[event_id]"]').attr('disabled','disabled');
  }
  ...
  $('.sf_admin_form .checkpoint_new form').submit(function(){
    $.post($(this).attr('action'),$(this).serialize()+'&_save_and_add=on',function(data){
